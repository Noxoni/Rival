"""Launch a standard offline human-vs-Rival-2 RLBot v5 match."""
...
RIVAL2_CONFIG = ROOT / "bot" / "rival2_live" / "rival2.bot.toml"
...
def build_match(human_team: int, launcher: str) -> flat.MatchConfiguration:
    ...
    rival = rlbot.config.load_player_config(RIVAL2_CONFIG, rival_team)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Play a standard five-minute offline 1v1 against Rival 2"
    )
    ...
    try:
        ...
        print(
            f"Rival 2 match started: human={'Blue' if human_team == 0 else 'Orange'}, "
            f"Rival 2={'Orange' if human_team == 0 else 'Blue'}"
        )
        while True:
