"""Train and evaluate one immutable M10.10 first-touch boundary."""
...
def run_boundary(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if boundary_hours not in BOUNDARY_HOURS:
        raise ValueError(f"Unsupported M10.10 boundary: {boundary_hours}")
    ...
    if preflight.get("status") != "passed" or preflight.get("ppo_authorized") is not True:
        raise RuntimeError("M10.10 PPO is forbidden until hard preflight passes")
    source = args.source_checkpoint.resolve()
    loaded = load_checkpoint(source, device=args.device, expected_config=config)
    state = loaded["trainer_state"]
    if state.get("m10_10_minimal_first_touch") is not True:
        raise RuntimeError("Source checkpoint is not in the M10.10 lineage")
    expected_source = {0.25: 0, 0.5: 108_000, 1.0: 216_000}[boundary_hours]
    if int(state["cumulative_agent_steps"]) < expected_source:
        raise RuntimeError("Source has not reached the preceding M10.10 boundary")
    ...
    try:
        ...
        while trainer.cumulative_agent_steps < target_steps:
            ...
            write_json_atomic(
                campaign_state_path,
                {
                    "format": "rival-m10-10-minimal-first-touch-state-v1",
                    "phase": "training",
    ...
    if cleanup is None or not cleanup["passed"]:
        raise RuntimeError(f"M10.10 worker cleanup failed: {cleanup}")
    if latest_held is None:
        raise RuntimeError("M10.10 boundary made no recoverable PPO update")
    ...
    result = {
        "schema_version": 1,
        "training_boundary_version": "RivalM10_10MinimalFirstTouchBoundaryV1",
    ...
    write_json_atomic(RESULT_ROOT / f"training_{slug}.json", result)
    write_json_atomic(
        campaign_state_path,
        {
            "format": "rival-m10-10-minimal-first-touch-state-v1",
    ...
    if not result["checks"]["passed"]:
        raise RuntimeError(f"M10.10 boundary failed: {result['checks']}")
    return result
...
def main() -> int:
    ...
    result = run_boundary(args)
    print(
        json.dumps(
            {
                "status": result["status"],
                "boundary_hours": result["boundary_hours"],
